...
public class DeliveryService {
    ...
    //// Handles database operations for delivery partner,delivery records, fetch and update delivery assignment
    ...
    public DeliveryPartnerResponse createDeliveryPartner(DeliveryPartnerRequest request) {
        ...
        DeliveryPartner partner = DeliveryPartner.builder().userId(request.getUserId()) //// Link delivery partner profile with auth-service user id
                .partnerName(request.getPartnerName()).phone(request.getPhone())// Store partner profile details.
                .vehicleNumber(request.getVehicleNumber())
                .partnerStatus(                         
                        request.getPartnerStatus() == null ? PartnerStatus.AVAILABLE : request.getPartnerStatus())   // If status is not provided, make partner available by default.
        ...
        log.info(
                "Delivery partner profile created successfully. partnerId={}, userId={}, status={}",
                savedPartner.getId(),
                savedPartner.getUserId(),
                savedPartner.getPartnerStatus()
        );
    ...
    public List<DeliveryPartnerResponse> getAllDeliveryPartners() {
        ...
        List<DeliveryPartnerResponse> partners = deliveryPartnerRepository.findAll()
                .stream()
  ...
  //All database operations inside this method should be treated as one single unit of work.(means no half completed db state.
    ...
    public DeliveryResponse assignDeliveryPartner(Long orderId) {
     ...
     // Select best available partner based on workload, distance and rating.
        ...
        log.info(
                "Best delivery partner selected. orderId={}, partnerId={}, activeDeliveryCount={}, rating={}",
                orderId,
                bestPartner.getId(),
                getSafeActiveDeliveryCount(bestPartner),
                getSafeRating(bestPartner)
        );
    ...
    //this method update delivery lifecycle method(ASSIGNED → PICKED_UP → OUT_FOR_DELIVERY → DELIVERED)
    @Transactional
    public DeliveryResponse updateDeliveryStatus(Long deliveryId, DeliveryStatusUpdateRequest request) {
        ...
        Order order = orderRepository.findById(delivery.getOrderId())
                .orElseThrow(() -> {
                    log.warn(
                            "Delivery status update failed because order was not found. orderId={}, deliveryId={}",
                            delivery.getOrderId(),
                            deliveryId
                    );
        ...
        if (newStatus == DeliveryStatus.PICKED_UP) {
            delivery.setPickedUpAt(LocalDateTime.now());  //// Store pickup timestamp and move order to out-for-delivery stage.
        ...
        if (newStatus == DeliveryStatus.OUT_FOR_DELIVERY) {
            delivery.setOutForDeliveryAt(LocalDateTime.now());//// Store out-for-delivery timestamp and keep order status in delivery stage.
            order.setOrderStatus(OrderStatus.OUT_FOR_DELIVERY);
        ...
        if (newStatus == DeliveryStatus.DELIVERED) {
            ...
            log.info(
                    "Delivery completed successfully. deliveryId={}, orderId={}, partnerId={}",
                    deliveryId,
                    order.getId(),
        ...
        if (newStatus == DeliveryStatus.CANCELLED) {
            ...
            log.warn(
                    "Delivery cancelled. deliveryId={}, orderId={}, partnerId={}",
                    deliveryId,
                    order.getId(),
                    delivery.getDeliveryPartner().getId()
            );
        ...
        log.info(
                "Delivery status updated successfully. deliveryId={}, orderId={}, status={}",
                savedDelivery.getId(),
    ...
    //It fetches only the currently active deliveries assigned to the logged-in delivery partner.
    @Transactional(readOnly = true)
    public List<DeliveryResponse> getActiveDeliveriesByPartnerUserId(Long userId) {
     ...
     // Resolve delivery partner profile using auth-service user id.
        ...
        // Only active/in-progress delivery statuses should be shown on delivery dashboard.
        ...
        log.info(
                "Active deliveries fetched successfully. userId={}, partnerId={}, activeDeliveryCount={}",
                userId,
                partner.getId(),
                activeDeliveries.size()
        );
    ...
    private void validateDeliveryPartnerRequest(DeliveryPartnerRequest request) {
        ...
        if (request.getPartnerName() == null || request.getPartnerName().trim().isEmpty()) {
            log.warn("Create delivery partner request failed because partnerName is blank. userId={}", request.getUserId());
            throw new RuntimeException("Partner name is required");
        }

        if (request.getCurrentLatitude() == null) {
            log.warn("Create delivery partner request failed because currentLatitude is null. userId={}", request.getUserId());
            throw new RuntimeException("Current latitude is required");
        }

        if (request.getCurrentLongitude() == null) {
            log.warn("Create delivery partner request failed because currentLongitude is null. userId={}", request.getUserId());
    ...
    private void validateOrderForDeliveryAssignment(Order order) {
        if (order.getPaymentStatus() != PaymentStatus.PAID) {
            log.warn(
                    "Delivery assignment validation failed because order is not paid. orderId={}, paymentStatus={}",
                    order.getId(),
        ...
        if (order.getOrderStatus() != OrderStatus.CONFIRMED) {
            log.warn(
                    "Delivery assignment validation failed because order is not confirmed. orderId={}, orderStatus={}",
                    order.getId(),
        ...
        if (order.getDeliveryLatitude() == null || order.getDeliveryLongitude() == null) {
            log.warn("Delivery assignment validation failed because delivery location is missing. orderId={}", order.getId());
            throw new RuntimeException("Order delivery location is missing");
        }
    }


    //this method will find the best delivery partner, it will check availability,rating, 
    //distance from restaurant using longitude and latitude along with the number of order delivered.
    ...
    private DeliveryPartner findBestAvailablePartner() {
        ...
        log.info(
                "Best available partner resolved. partnerId={}, activeDeliveryCount={}, rating={}",
                selectedPartner.getId(),
                getSafeActiveDeliveryCount(selectedPartner),
                getSafeRating(selectedPartner)
        );
    ...
    //this method is used to release the delivery partner after delivery is completed or cancelled.
    private void makePartnerAvailable(DeliveryPartner partner) {
        ...
        log.info(
                "Delivery partner marked available. partnerId={}, activeDeliveryCount={}",
                partner.getId(),
    ...
    private int getSafeActiveDeliveryCount(DeliveryPartner partner) {
    	// Treat missing active delivery count as zero for assignment and availability logic.
    ...
    //this method calculates distance in kilometers between two latitude/longitude points using the Haversine formula.
